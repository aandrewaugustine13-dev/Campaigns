#!/usr/bin/env npx tsx
// Throwaway inspector for Stage 1 cast. Prints the proposed cast JSON to
// stdout and diagnostics to stderr, so you can compare standards directly:
//   npm run cast -- "TEKS ... Erie Canal"
//   npm run cast -- "TEKS ... Lewis and Clark"
// or pipe a clean JSON file:  npm run cast -- "..." > erie-cast.json
...
async function main() {
  ...
  console.error(`\n→ Proposing cast for standard:\n  ${standard}\n`);

  const { data, findings } = await generateCast(standard, apiKey);
